...
PACK_CAST = re.compile(r"^Add To Stack: Ai\(\d+\)-Hunting Storm cast Hunting Pack\b.*$", re.I | re.M)
...
def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("--jar",required=True,type=Path); ap.add_argument("--deck-dir",default=Path.home()/".forge/decks/constructed",type=Path); ap.add_argument("--output",required=True,type=Path); ap.add_argument("--start-seed",type=int,default=74001)
    args=ap.parse_args(); args.output.mkdir(parents=True,exist_ok=True); deck_dir=args.deck_dir.expanduser().resolve(); jar=args.jar.resolve()
