...
def main():
    ...
    for build, jar in (("stock", args.stock.resolve()), ("patched", args.patched.resolve())):
        ...
        for idx, (deck, opp, a, b) in enumerate(MATCHUPS, 1):
            for seed in SEEDS:
                ...
                w = winner(text)
                if not w: bad.append("unparsed_game")
                if bad: failures.append({"build": build, "log": str(log), "issues": bad})
                ...
                rows.append({"build": build, "deck": deck, "opponent": opp, "orientation": f"{Path(a).stem}__vs__{Path(b).stem}",
                             ...
                             "log": str(log.relative_to(args.output)), "command": cmd, **event_row})
    ...
    # Small-sample directional performance only; no significance language.
    perf = []
    ...
    # If all target event counters are exactly identical stock vs patched for a deck, relevant behavior is indistinguishable by this observable telemetry.
    indist = []
